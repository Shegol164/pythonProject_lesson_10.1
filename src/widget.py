...
def mask_account_card(card_or_account: str) -> [str]:
    ...
    if card_or_account.lower().startswith("счет"):
        print(f"{card_or_account_type} {get_mask_account(card_or_account_splitted[-1])}")
    else:
        print(f"{card_or_account_type} {get_mask_card_number(card_or_account_splitted[-1])}")
...
def get_date(date: str) -> str:
    ...
    new_data = now.strftime("%d.%m.%YT%H:%M:%S.%f")
    return new_data
